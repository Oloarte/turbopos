...
func main() {
	...
	cfdiConn, err := grpc.Dial(getenv("CFDI_ADDR", "localhost:50053"),
		grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		log.Fatalf("ERROR conectando cfdi: %v", err)
	}
...
func (gw *Gateway) handleLogs(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		if cfdiUUID.Valid {
			entry["cfdi_uuid"] = cfdiUUID.String
		}
...
func (gw *Gateway) handleCorte(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(map[string]interface{}{
		...
		"total_timbradas":     totalTimbradas,
		"por_metodo":          metodos,
